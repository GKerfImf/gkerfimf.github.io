...
(** In the following, we define the notion of arbitrary arrival
    curves, which can be used to reason about the frequency of job
    arrivals. In contrast to the sporadic task model, the arbitrary
    arrival curves model is well-suited to expressing bursty and
    irregular arrival processes. *)

(** ** Task Parameters for the Arrival Curves Model *)

(** The arrival curves model describes tasks by giving an upper bound
    and, optionally, a lower bound on the number of new job arrivals
    during any given interval. These bounds are typically called
    arrival curves. *)

(** We let [max_arrivals tsk Δ] denote a bound on the maximum number
    of arrivals of jobs of task [tsk] in any interval of length
    [Δ]. *)
Class MaxArrivals (Task : TaskType) := max_arrivals : Task -> duration -> nat.

(** Conversely, we let [min_arrivals tsk Δ] denote a bound on the
    minimum number of arrivals of jobs of task [tsk] in any interval
    of length [Δ]. *)
Class MinArrivals (Task : TaskType) := min_arrivals : Task -> duration -> nat.

(** Alternatively, it is also possible to describe arbitrary arrival
    processes by specifying the minimum and maximum lengths of an
    interval in which a given number of jobs arrive. Such bounds are
    typically called minimum- and maximum-distance functions. *)

(** We let [min_separation tsk N] denote the minimal length of an
    interval in which exactly [N] jobs of task [tsk] arrive. *)
Class MinSeparation (Task : TaskType) := min_separation : Task -> nat -> duration.

(** Conversely, we let [max_separation tsk N] denote the maximal
    length of an interval in which exactly [N] jobs of task [tsk]
    arrive. *)
...
(** In the following, we precisely define the semantics of the arrival
    curves model. *)
...
Section ArrivalCurvesModel.
  ...
  arrival and separation curves to the entire task set.  *)
