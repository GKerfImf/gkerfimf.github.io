...
Section BusyIntervalJLFP.
  ...
  Section BusyInterval.
    ...
    (** We say that [t] is a quiet time for [j] iff every
        higher-priority job from the arrival sequence that arrived
        before [t] has completed by that time. *)
    ...
    (** Based on the definition of quiet time, we say that interval
         <<[t1, t_busy)>> is a (potentially unbounded) busy-interval
         prefix iff the interval starts with a quiet time where a
         higher or equal priority job is released and remains
         non-quiet. We also require job [j] to be released in the
         interval. *)    
    ...
    (** Next, we say that an interval <<[t1, t2)>> is a busy interval
         iff <<[t1, t2)>> is a busy-interval prefix and [t2] is a
         quiet time. *)
