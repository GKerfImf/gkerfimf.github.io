...
Section ExistsBusyIntervalJLFP.
  ...
  Section BoundingBusyInterval.
    ...
    (** Next, we recall the notion of workload of all jobs released in
        a given interval <<[t1, t2)>> that have higher-or-equal priority
        w.r.t the job j being analyzed. *)
    Let hp_workload t1 t2 :=
      workload_of_higher_or_equal_priority_jobs j (arrivals_between t1 t2).

    (** With regard to the jobs with higher-or-equal priority that are
           released in a given interval <<[t1, t2)>>, we also recall
           the service received by these jobs in the same interval
           <<[t1, t2)>>. *)
    ...
    Section BoundingBusyInterval.
      ...
      Section UpperBound.
        ...
        (** Next, assume that for some positive delta, the sum of
               requested workload at time <<[t1 + delta]>> and
               constant priority_inversion_bound is bounded by delta
               (i.e., the supply). *)
        ...
        (** If there is a quiet time by time <<[t1 + delta]>>, it
               ...
               Thus, let's consider first the harder case where there
               is no quiet time, which turns out to be impossible. *)
        Section CannotBeBusyForSoLong.
          ...
          (** Since the interval is always non-quiet, the processor is
              ...
              <<[t1, t1 + delta)>> and priority inversion equals delta. *)
