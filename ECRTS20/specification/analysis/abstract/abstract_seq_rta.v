...
(** In this section we propose the general framework for response-time
  ...
  (** We prove that the maximum among the solutions of the
     response-time bound recurrence for some set of parameters is a
     response-time bound for [tsk].  Note that in this section we _do_
     rely on the hypothesis about task sequentiality. This allows us
     to provide a more precise response-time bound function, since
     jobs of the same task will be executed strictly in the order they
     arrive. *)
Section Sequential_Abstract_RTA.
  ...
  Section Definitions.
    ...
    Definition task_interference_received_before
               (tsk : Task) (upper_bound : instant) (t : instant) :=
    ...
    (** We say that task interference is bounded by
       [task_interference_bound_function] iff for any job [j] of task
       [tsk] cumulative _task_ interference within the interval <<[t1,
       t1 + R)>> is bounded by function
       [task_interference_bound_function(tsk, A, R)]. Note that this
       definition is almost the same as the definition of
       [job_interference_is_bounded_by] from the
       non-necessary-sequential case. However, in this case we ignore
       the interference that comes from jobs from the same task. *)
  ...
  Section ResponseTimeBound.
    ...
    Section BoundOfCumulativeJobInterference.
      ...
      (** In this section, we show that the cumulative interference of
         ...
         <<[t1, t1 + x)>>. Note that the task workload is computed
         only on the interval <<[t1, t1 + A + ε)>>. Thanks to the
         hypothesis about sequential tasks, jobs of task [tsk] that
         arrive after [t1 + A + ε] cannot interfere with [j]. *)
      Section TaskInterferenceBoundsInterference.

        (** We start by proving a simpler analog of the lemma which
        ...
        (** Finally, we show that the cumulative interference of job [j]
           in the interval <<[t1, t1 + x)>> is bounded by the sum of the
           task workload in the interval <<[t1, t1 + A + ε)>> and the
      ...
      (** In order to obtain a more convenient bound of the cumulative
         interference, we need to abandon the actual workload in favor
         of a bound which depends on task parameters only.  So, we
         show that actual workload of the task excluding workload of
         any job [j] is no greater than bound of workload excluding
    ...
    (** Finally, we apply the [uniprocessor_response_time_bound]
       ...
       requirements are satisfied. So, [R] is a response time
       bound. *)
