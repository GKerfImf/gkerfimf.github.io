...
(** We prove that the maximum (with respect to the set of offsets)
   ...
   response time bound for [tsk]. Note that in this section we do not
   rely on any hypotheses about job sequentiality. *)
Section Abstract_RTA.
  ...
  (** ...and a valid task run-to-completion threshold function. That
     is, [task_run_to_completion_threshold tsk] is (1) no bigger than
     [tsk]'s cost, (2) for any job of task [tsk] job's
     run-to-completion threshold is bounded by task's
  ...
  (** Consider any value [R] that upper-bounds the solution of each
     ...
     such that [F + (task_cost tsk - task_run_to_completion_threshold
     tsk) <= R]. *)
  Variable R: nat.
  ...
  (** In this section we show a detailed proof of the main theorem
     that establishes that [R] is a response-time bound of task
     [tsk]. *) 
  Section ProofOfTheorem.
    ...
    (** In order to prove that R is a response-time bound of job j, we
       use hypothesis [H_R_is_maximum].  Note that the relative
       arrival time ([A]) is not necessarily from the search
       space. However, earlier we have proven that for any A there
       exists another [A_sp] from the search space that shares the
       same IBF value. Moreover, we've also shown that there exists an
       [F_sp] such that [F_sp] is a solution of the response time
       recurrence for parameter [A_sp]. Thus, despite the fact that
       the relative arrival time may not lie in the search space, we
       can still use the assumption [H_R_is_maximum]. *)
    ...
    (** (d) [A_sp + F_sp] is a solution of the response time
        recurrence... *)
    ...
    (** In this section, we consider the case where the solution is so
       large that the value of [t1 + A_sp + F_sp] goes beyond the busy
       interval. Although this case may be impossible in some
       scenarios, it can be easily proven, since any job that
       completes by the end of the busy interval remains completed. *)
    Section FixpointOutsideBusyInterval.

      (** By assumption, suppose that [t2] is less than or equal to
          [t1 + A_sp + F_sp]. *)
    ...
    Section FixpointInsideBusyInterval.
      ...
      (** CASE 1: the value of the fix-point is no less than the
          relative arrival time of job [j]. *)
      Section FixpointIsNoLessThanArrival.
        ...
        Section ProofByContradiction.
          ...
          (** Some additional reasoning is required since the term
             [task_cost tsk - task_run_to_completion_threshold tsk]
             does not necessarily bound the term [job_cost j -
             job_run_to_completion_threshold j]. That is, a job can
             have a small run-to-completion threshold, thereby
             becoming non-preemptive much earlier than guaranteed
             according to task run-to-completion threshold, while
             simultaneously executing the last non-preemptive segment
             that is longer than [task_cost tsk -
             task_run_to_completion_threshold tsk] (e.g., this is
             possible in the case of floating non-preemptive
             sections).

             In this case we cannot directly apply lemma
             "j_receives_at_least_run_to_completion_threshold". Therefore
             we introduce two temporal notions of the last
             non-preemptive region of job j and an execution
             optimism. We use these notions inside this proof, so we
             define them only locally. *)

          (** Let the last non-preemptive region of job [j] (last) be
             the difference between the cost of the job and the [j]'s
             run-to-completion threshold (i.e. [job_cost j -
             job_run_to_completion_threshold j]).  We know that after
             j has reached its run-to-completion threshold, it will
             additionally be executed [job_last j] units of time. *)          
          ...
          (** From lemma
             [j_receives_at_least_run_to_completion_threshold] with
             parameters [progress_of_job :=
             job_run_to_completion_threshold j] and [delta := (A + F)
             - optimism)] we know that service of [j] by time [t1 + (A
             + F) - optimism] is no less than
             [job_run_to_completion_threshold j]. Hence, job [j] is
             completed by time [t1 + (A + F) - optimism + last]. *)
          ...
          Section AuxiliaryInequalities.
            ...
            (** Recall that we consider a busy interval of a job [j],
               and [j] has arrived [A] time units after the beginning
               the busy interval. From basic properties of a busy
               interval it follows that job [j] incurs interference at
               any time instant [t] ∈ <<[t1, t1 + A)>>.  Therefore
               [interference_bound_function(tsk, A, A + F)] is at
               least [A]. *)
          ...
          (** Next we show that [t1 + (A + F) - optimism + last] is at
             most [job_arrival j + R], which is easy to see from the
      ...
      Section FixpointCannotBeSmallerThanArrival.
        ...
        (** We can use
            [j_receives_at_least_run_to_completion_threshold] to prove
            that the service received by [j] by time [t1 + (A_sp +
            F_sp)] is no less than run-to-completion threshold. *)
