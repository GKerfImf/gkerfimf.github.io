...
Section AbstractRTAforFPwithArrivalCurves.
  ...
  (** Consider an FP policy that indicates a higher-or-equal priority
     relation, and assume that the relation is reflexive. Note that we
     do not relate the FP policy with the scheduler. However, we
     define functions for Interference and Interfering Workload that
     actively use the concept of priorities. We require the FP policy
     to be reflexive, so a job cannot cause lower-priority
     interference (i.e. priority inversion) to itself. *)
  ...
  (** Let R be a value that upper-bounds the solution of each
     ...
     in the search space, there exists a corresponding solution F such
     that [F + (task_cost tsk - task_run_to_completion_threshold tsk) <= R]. *)
  Variable R : duration.
  ...
  Section FillingOutHypothesesOfAbstractRTATheorem.
    ...
    (** First, we prove that in the instantiation of interference and
       interfering workload, we really take into account everything
       that can interfere with [tsk]'s jobs, and thus, the scheduler
       satisfies the abstract notion of work conserving schedule. *) 
    ...
    (** Recall that [L] is assumed to be a fixed point of the busy
       ...
       busy interval and the one obtained from the abstract definition
       (with the interference and interfering workload)
    ...
    (** Next, we prove that [IBF] is indeed an interference bound.
       ...
       task t, and the length of the interval to the maximum amount of
       interference (for more details see files
       [abstract_RTA.definitions] and
       [abstract_RTA.abstract_seq_rta]).

       However, in this module we analyze only one task -- [tsk],
       therefore it is “hard-coded” inside the interference bound
       function [IBF]. Moreover, in case of a model with fixed
       priorities, interference that some job [j] incurs from
       higher-or-equal priority jobs does not depend on the relative
       arrival time of job [j]. Therefore, in order for the [IBF]
       signature to match the required signature in module
       [abstract_seq_RTA], we wrap the IBF function in a function that
       accepts, but simply ignores, the task and the relative arrival
       time. *)
